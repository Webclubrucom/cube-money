          ...
          <div class="modal-content">
            <form action="" method="post" id="form">
              ...
              <div class="modal-footer">
                ...
                <input type="submit" class="btn btn-primary btn-fill" id="dodeposit" value="<?php echo trans('lang.save');?>"/>
                <button type="button" class="btn btn-danger" data-dismiss="modal"><?php echo trans('lang.close');?></button>
              </div>
            </form>
          </div>
          ...
          <div class="modal-content">
            <form action="" method="post" id="form">
              ...
              <div class="modal-body">
                ...
                <div class="row">
					...
					<div class="form-group col-lg-6" id="targetdate">
									...
									<small class="req text-danger">* </small><?php echo trans('lang.target_date');?></label>
									<div  class="input-group date" data-date-format="mm-dd-yyyy">
										<input id="tdate" class="form-control" type="text" value="<?php echo date("d.m.Y");?>"/>
										<span class="input-group-addon" style="border: 1px solid #cecece;"><i class="fa fa-calendar"></i></span>
									</div>
					</div>
				</div>
              ...
              <div class="modal-footer">
                ...
                <input type="submit" class="btn btn-primary btn-fill" id="save" value="<?php echo trans('lang.save');?>"/>
                <button type="button" class="btn btn-danger" data-dismiss="modal"><?php echo trans('lang.close');?></button>
              </div>
            </form>
          </div>
...
$(document).ready(function() {
	...
	$("#save").click(function(e){
		...
		$.ajax({
			type: "POST",
            url: "{{ url('goals/save')}}",
            data: {name:name,opening:opening,target:target,deadline:deadline.split(".").reverse().join("-")},
            dataType: "JSON",
	...
	$("#saveedit").click(function(e){
		...
		$.ajax({
            ...
            url: "{{ url('goals/edit')}}",
			dataType: "JSON",
            data: {id:id,name:name,opening:opening,target:target,date:date.split(".").reverse().join("-")},
            success: function(data) {
				$("#message2").css({'display':"block"});
				window.setTimeout(function(){location.reload()},2000)
            }
		...
		$('#edit').on('show.bs.modal', function(e) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					$("#edate").val(data.message[0].deadline.split("-").reverse().join("."));
