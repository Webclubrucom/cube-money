...
class ReportsController extends Controller
{
	...
	public function gettransactions(Request $request){
		...
		->addColumn('amount',function($single){
				...
				return number_format($single->amount,2).' '.$setting[0]->currency;
			})
	...
	public function getaccounttransaction(Request $request){
		...
		->addColumn('income',function($single){
				...
				if($single->income =='-'){
					...
					$return = '<p class="text-success">'.number_format($single->income,2).' '.$setting[0]->currency.'</p>';	
				}
				return $return;
			})
		->addColumn('expense',function($single){
				...
				if($single->expense =='-'){
					...
					$return = '<p class="text-danger">'.number_format($single->expense,2).' '.$setting[0]->currency.'</p>';	
				}
				return $return;
			})
		->addColumn('transactiondate',function($single){
	...
	public function getincomemonthly(){
		...
		->addColumn('ijan',function($single){
				$setting = DB::table('settings')->where('settingsid','1')->get();
				return number_format($single->ijan,2).' '.$setting[0]->currency;
			})
		->addColumn('ifeb',function($single){
				$setting = DB::table('settings')->where('settingsid','1')->get();
				return number_format($single->ifeb,2).' '.$setting[0]->currency;
			})
		->addColumn('imar',function($single){
				$setting = DB::table('settings')->where('settingsid','1')->get();
				return number_format($single->imar,2).' '.$setting[0]->currency;
			})
		->addColumn('iapr',function($single){
				$setting = DB::table('settings')->where('settingsid','1')->get();
				return number_format($single->iapr,2).' '.$setting[0]->currency;
			})
		->addColumn('imay',function($single){
				$setting = DB::table('settings')->where('settingsid','1')->get();
				return number_format($single->imay,2).' '.$setting[0]->currency;
			})
		->addColumn('ijun',function($single){
				$setting = DB::table('settings')->where('settingsid','1')->get();
				return number_format($single->ijun,2).' '.$setting[0]->currency;
			})
		->addColumn('ijul',function($single){
				$setting = DB::table('settings')->where('settingsid','1')->get();
				return number_format($single->ijul,2).' '.$setting[0]->currency;
			})
		->addColumn('iags',function($single){
				$setting = DB::table('settings')->where('settingsid','1')->get();
				return number_format($single->iags,2).' '.$setting[0]->currency;
			})
		->addColumn('isep',function($single){
				$setting = DB::table('settings')->where('settingsid','1')->get();
				return number_format($single->isep,2).' '.$setting[0]->currency;
			})
		->addColumn('iokt',function($single){
				$setting = DB::table('settings')->where('settingsid','1')->get();
				return number_format($single->iokt,2).' '.$setting[0]->currency;
			})
		->addColumn('inov',function($single){
				$setting = DB::table('settings')->where('settingsid','1')->get();
				return number_format($single->inov,2).' '.$setting[0]->currency;
			})
		->addColumn('idec',function($single){
				$setting = DB::table('settings')->where('settingsid','1')->get();
				return number_format($single->idec,2).' '.$setting[0]->currency;
			})
		->addColumn('total',function($single){
				$setting = DB::table('settings')->where('settingsid','1')->get();
				return number_format($single->total,2).' '.$setting[0]->currency;
			})
