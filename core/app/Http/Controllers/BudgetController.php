...
class BudgetController extends Controller
{
	...
	public function getdata() {
		...
		->addColumn( 'fromdate', function( $single ) {
				...
				$mec = $_monthsList[date("M", strtotime( $single->fromdate ))];
				
					return $mec.' '.date("Y", strtotime( $single->fromdate ));
					//return date("Y", strtotime( $single->fromdate ));
	...
	public function gettransactionbydate( Request $request ) {
		...
		//$onlyconsonants = str_replace($_monthsList, $_months, $request->input( 'date' ));
				
		$date   = str_replace($_monthsList, $_months, $request->input( 'date' ));
		...
		if ( count( $data )==0 ) {
			...
			$res['amountcurrency'] = number_format( $data[0]->totalamount, 2 ).' '.$setting[0]->currency;
