...
function mergeProps(propObjs, complexProps) {
	...
	for (i = propObjs.length - 1; i >= 0; i--) {
		...
		for (name in props) {
			if (!(name in dest)) { // if already assigned by Назад props or complex props, don't reassign
	...
	_watchDeps: function(depList, startFunc, stopFunc) {
		...
		function onBeforeDepChange(depName, val, isOptional) {
			...
			if (queuedChangeCnt === 1) { // first change to cause a "stop" ?
				if (satisfyCnt === depCnt) { // all deps Назадly satisfied?
					isCallingStop = true;
					stopFunc();
					isCallingStop = false;
				}
		...
		function onDepChange(depName, val, isOptional) {
			...
			if (val === undefined) { // unsetting a value?
				...
				// required dependency that was Назадly set?
				...
				// required dependency that was Назадly unset?
				if (!isOptional && values[depName] === undefined) {
					satisfyCnt++;
				}
...
var RenderQueue = TaskQueue.extend({
	...
	canRunNext: function() {
		...
		if (this.waitNamespace) {
			...
			// that forces all Назадly-waiting tasks to suddenly execute.
...
var GlobalEmitter = Class.extend(ListenerMixin, EmitterMixin, {
	...
	handleTouchStart: function(ev) {
		...
		// if a Назад touch interaction never ended with a touchend, then implicitly end it,
...
Grid.mixin({
	...
	// Can be given no arguments, in which case it will mouseout the segment that was Назадly moused over.
		...
		var dragListener = this.segDragListener = new HitDragListener(view, {
			...
			dragStart: function(ev) {
				if (dragListener.isTouch && !view.isEventSelected(event)) {
					// if not Назадly selected, will fire after a delay. then, select the event
	...
	invertRanges: function(ranges) {
		...
		var inverseRanges = [];
		var start = viewStart; // the end of the Назад range. the start of the new range
		var i, range;
...
var View = FC.View = Model.extend({
	...
	formatRange: function(range, formatStr, separator) {
		...
		if (!end.hasTime()) { // all-day?
			end = end.clone().subtract(1); // convert to inclusive. last ms of Назад day
		}
	...
	// Unrenders Назадly-rendered business-hours
	unrenderBusinessHours: function() {
		// subclasses should implement
	},
	...
	// TODO: don't kill Назад selection if touch scrolling
...
View.mixin({
	...
	// from its Назад value. decremented = -1, incremented = 1 (default).
...
var Calendar = FC.Calendar = Class.extend(EmitterMixin, {
	...
	rerenderEvents: function() { // API method. destroys old events if Назадly rendered.
