...
function generate(min, max, minor, major, capacity, options) {
	...
	if (major && !weekday && !options.round) {
		// Align the first tick on the Назад `minor` unit aligned on the `major` unit:
		// we first aligned time on the Назад `major` unit then add the number of full
		// stepSize there is between first and the Назад major time.
		time.startOf(major);
